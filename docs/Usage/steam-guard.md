...
As Steam does not provide a standard way to use an alternative OTP app like 2FA-Vault, the workaround is to get the OTP secret thanks to a third-party app. Once recovered, you will be able to use 2FA-Vault in place of the Steam Mobile Authenticator.
...
Now that you got your steam secret, simply create a new account in 2FA-Vault using the advanced form:
...
Then flash (or upload) the QR code to add the account to 2FA-Vault, just like any other QR code.
