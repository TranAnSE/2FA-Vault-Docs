...
2FA-Vault lets you decorate your 2FA accounts with icons. It can automatically fetch icons from several sources, helping you build an appealing 2FA dashboard with minimal effort.

:::desktop-screen
![2FA-Vault dashboard with icons](/static/icon-dashboard.png)
:::
...
Not a fan of icons? That's fine. Go to _Settings > Options_ and uncheck [!badge size="l" icon="checkbox" text="Show icons"]. 2FA-Vault will stop showing them next to the 2FA account names on the main view.
...
Enable the [!badge size="l" icon="checkbox" text="Get official icons"] preference at _Settings > Options_ if you want 2FA-Vault to fetch 2FA icons automatically. You can then choose whether they are fetched from an online icon collection or from an uploaded icon pack.
...
!!!question Icon matching - How does 2FA-Vault get an icon that matches a newly added 2FA account?
...
As mentioned above, 2FA-Vault can query online icon collections. Several collections are available out of the box, so you only need to select your preferred one before fetching your first icon.
...
Online icon collections may offer several flavors of their icons. The [!badge size="l" icon="single-select" iconAlign="right" text="Icon variant"] preference lets you choose which variation 2FA-Vault should fetch first. Unless you enable [!badge size="l" icon="checkbox" text="Strict fetch"], the regular variant will be used as a fallback if the selected variant is unavailable at the icon source, given that the regular variant should be always available.

:::desktop-screen
![The 2FA-Vault icon in Light and Dark flavor at dashboardicons.com](/static/icon-2fauth-variants.png)
:::
...
Starting with 2FA-Vault v6, it is also possible to use an icon pack as a source of icons.
...
2FA-Vault supports the following image formats for icon pack files: `.png`, `.svg`, `.webp`, `.jpg|jpeg`, `.bmp`
!!!

### Icon pack vs Online collection

Whether you use online collections or icon packs, 2FA-Vault searches them the same way to find a matching icon. The key difference is that icon packs are stored locally on the 2FA-Vault server. This means 2FA-Vault does not make outgoing requests to fetch icons — ideal for instances in closed networks.
...
- Icons in packs must be correctly named for 2FA-Vault to identify matches.
...
You need read/write access to the server hosting 2FA-Vault to perform this action.
!!!

Icon packs must be stored in `[twofauth_install_dir]/storage/app/iconPacks/`. Making an icon pack available is as simple as copying its folder to that location. If you run 2FA-Vault in a Docker container, the directory `storage/app/iconPacks/` should be part of the bind-mounted volume you configured. Create it if it doesn't exist.
...
Sometimes designers provide icons in different flavors. 2FA-Vault supports this via subdirectories.
...
To make them both available in 2FA-Vault, dispatch the icons into 2 subdirectories: one for the dark icons and one for the light icons. You end-up with `storage/app/iconPacks/Social-Media/Dark/` and `storage/app/iconPacks/Social-Media/Ligth/`.
