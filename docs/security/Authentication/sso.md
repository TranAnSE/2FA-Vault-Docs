...
In other words, you can use an existing account, say your github account, to authenticate to 2FA-Vault.
...
!!!info
For now 2FA-Vault only supports 2 SSO providers: __OpenID__ and __Github__
!!!
...
You have to create a client ID on the provider side first. This is required so that your 2FA-Vault instance is considered legit when requesting an access token to the provider.
...
- The Authorization callback URL is to build as:  
  `[your_twofauth_url]/socialite/callback/[the_provider_name]`

> __Example__:  
>_Your 2FA-Vault instance url is `https://2FA-Vault.mydomain.com`  
>_Your provider is __Github__  
>
> Then your callback URL is:  
> `https://2FA-Vault.mydomain.com/socialite/callback/github`

At the end of the process, you should be provided with a Client ID & Secret. Copy them as they are needed to set up the provider on the 2FA-Vault side.
...
Setting up a provider is done by defining its dedicated environment variables on your 2FA-Vault instance. You can find these vars in the `.env` file of 2FA-Vault. See also the [SSO settings](/getting-started/config/env-vars/#sso-setting) section.
...
Once a provider is enabled, a button to _Continue with_ this provider is available on the 2FA-Vault's Login page.
...
2. You will be prompted to grant permissions to 2FA-Vault to access your account
3. Regardless of your choice, you will be redirected back to 2FA-Vault
4. If you have granted access, you will be authenticated. If not, you will be back to the login form

!!!warning
You cannot sign in via SSO with a provider account that uses an email already registered on 2FA-Vault. Accounts cannot be merged.
!!!

### Registered without registering

When you sign in via SSO for the first time, you are registered to 2FA-Vault transparently. This means you own a 2FA-Vault user account on the instance but this account is bound to the provider account with the following restrictions:
...
- The 2FA-Vault account cannot be unbound from the provider account.
- You cannot change your information from 2FA-Vault. But changes made on the provider side are reflected on 2FA-Vault each time you sign in via SSO.

---

## Use SSO only

SSO can be set as the only authentication method available on your 2FA-Vault instance. Go to _Admin > Auth_, scroll down to the _Single Sign-On_ section and check [!badge size="l" icon="checkbox" text="Use SSO only"].
...
!!!warning
It is imperative that PATs be allowed for the [2FA-Vault browser extension](/#browser-extensions) to function properly.
!!!
...
As an administrator, you can fully disable Single Sign-On from the 2FA-Vault UI.
